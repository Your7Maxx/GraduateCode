...
if args.print_uid:
    print("%-6s" % ("UID"), end="")
print("%-7s %-16s %-16s %-7s %-16s %-7s %3s %s" % ("UID", "TIME", "PCOMM", "PID", "PPCOMM", "PPID", "RET", "ARGS"))
...
def write_event_data_to_file(event):
    with open("/root/commandDetect/kg/command_data.test", "ab") as file:  # 注意这里的 "ab"，表示以二进制追加模式打开文件
        event_time = str(datetime.datetime.fromtimestamp(time.time()).strftime("%Y-%m-%d %H:%M:%S"))
        ppid = event.ppid if event.ppid > 0 else get_ppid(event.pid)
        ppid = b"%d" % ppid if ppid > 0 else b"?"
        ppid_comm = get_comm(int(ppid)).encode()
        uid = event.uid

        argv_text = b'"' + b' '.join(argv[event.pid]).replace(b'\n', b'\\n') + b'"'
        argv_parts = argv_text.split(b' ')
        argv_parts[0] = event.comm
        argv_text = b' '.join(argv_parts)

        file.write(b'"%d" "%s" "%s" "%d" "%s" "%s" "%d" "%s\n' % (uid, event_time.encode(), event.comm, event.pid,
                                               ppid_comm, ppid, event.retval, argv_text))
# process event
def print_event(cpu, data, size):
    ...
    if event.type == EventType.EVENT_ARG:
        ...
        if not skip:
            ...
            printb(b"%-7d %-16s %-16s %-7d %-16s %-7s %3d %s" % (uid, event_time.encode(), event.comm, event.pid,
                   ppid_comm, ppid, event.retval, argv_text))

            write_event_data_to_file(event)
